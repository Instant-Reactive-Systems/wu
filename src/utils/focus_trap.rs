...
pub fn create_focus_trap(
    selector_id: impl Into<Cow<'static, str>>,
        ...
        (false, true) => {
            if let Some((start, end)) = children {
                if end
                    == &evt
                        .target()
                        .expect("keyboard event should have a target")
                        .dyn_into::<Node>()
                        .unwrap()
                {
                    let _ = start
