...
pub fn StackContext<M: 'static, T: Clone + 'static>(
    ...
    let active_cx = Signal::derive(move || stack_cxs.with(move |cxs| {
        { !cxs.is_empty() }.then(move || cxs[cxs.len() - 1].clone())
    }));
    ...
    provide_context(ActiveStackContext::<M, T> { cx: active_cx, _phant: Default::default() });
